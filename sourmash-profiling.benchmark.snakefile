...
rule sourmash_gather:
    ...
    benchmark: repeat(os.path.join(benchmarks_dir, "gather", "{sample}.k{ksize}.gather.benchmark"), 3)
    conda: "conf/env/sourmash.yml"
    shell:
...
rule tax_metagenome:
    ...
    benchmark: repeat(os.path.join(benchmarks_dir, "tax_metagenome", "{sample}.k{ksize}.tax_metagenome.benchmark"), 3)
    shell:
        """
        mkdir -p {params.outd}
...
rule tax_annotate:
    ...
    benchmark: repeat(os.path.join(benchmarks_dir, "tax_annotate", "{sample}.k{ksize}.tax_annotate.benchmark"), 3)
    shell:
        """
        mkdir -p {params.outd}
